...
import { FaStar, FaMapMarkerAlt, FaClock, FaHeart, FaImages, FaEuroSign, FaCheckCircle, FaCalendarAlt } from 'react-icons/fa';
...
interface CoiffeurCardProps {
  coiffeur: User & { availabilityStatus?: 'now' | 'in_hour' | 'today' | 'unavailable' };
...
const CoiffeurCard: React.FC<CoiffeurCardProps> = ({
  ...
  // ✅ Utiliser le hook centralisé pour vérifier le rôle
          ...
          <div className={`p-4 ${viewMode === 'list' ? 'flex-1' : ''}`}>
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                {/* Badge de disponibilité avec icônes CSS */}
                {coiffeur.availabilityStatus && coiffeur.availabilityStatus !== 'unavailable' && (
                  <div className="mb-3">
                    {coiffeur.availabilityStatus === 'now' && (
                    ...
                    {coiffeur.availabilityStatus === 'in_hour' && (
                      <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 border border-blue-300 shadow-sm">
                        <FaClock className="text-blue-600" />
                        Dans l'heure
                      </span>
                    )}
                    {coiffeur.availabilityStatus === 'today' && (
                      <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800 border border-yellow-300 shadow-sm">
