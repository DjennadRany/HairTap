...
import { LazyVideo } from './LazyVideo'; // ✅ Composant vidéo avec lazy loading
...
export const GalleryHub: React.FC<GalleryHubProps> = ({ className = "" }) => {
  ...
  const fetchServices = async (page = 1, append = false) => {
    try {
      ...
      // ✅ OPTIMISATION: Récupérer tous les coiffeurs d'abord
      const coiffeurs = await coiffeurService.searchCoiffeurs({});
      
      // ✅ OPTIMISATION: Récupérer les services de TOUS les coiffeurs EN PARALLÈLE (au lieu de séquentiel)
      ...
      // ✅ OPTIMISATION: Attendre tous les appels en parallèle
      const servicesArrays = await Promise.all(servicesPromises);
      const allServices = servicesArrays.flat(); // Aplatir les tableaux
      
      // ✅ OPTIMISATION: Supprimer les logs de debug en production (réduit la latence)
  ...
  // Obtenir le premier média disponible (priorité aux vidéos, puis images)
  const getServiceImage = (service: Service) => {
    if (service.gallery && service.gallery.length > 0) {
      // ✅ CORRECTION: Prioriser les vidéos, puis les images
      const video = service.gallery.find(item => item.mediaType === 'video');
      const image = service.gallery.find(item => item.mediaType === 'image' || !item.mediaType);
      
      // Si une vidéo existe, la retourner en priorité
      if (video) {
        const mediaUrl = video.mediaUrl || video.photoUrl;
        return getImageUrl(mediaUrl, DEFAULT_SERVICE_IMAGE);
      }
      
      // Sinon, retourner la première image
      if (image) {
        const mediaUrl = image.mediaUrl || image.photoUrl;
        return getImageUrl(mediaUrl, DEFAULT_SERVICE_IMAGE);
      }
      
      // Fallback: premier élément de la galerie
        ...
        <div className="gallery-masonry">
          {sortedServices.map((service) => (
            ...
            >
              ...
              {(() => {
                // ✅ CORRECTION: Vérifier si le service a des vidéos dans sa galerie (pas seulement le premier élément)
                const hasVideo = service.gallery && service.gallery.some(item => item.mediaType === 'video');
                ...
                return hasVideo || isVideoByUrl;
              })() ? (
                <LazyVideo
                  src={getServiceImage(service)}
                  className="w-full h-full object-cover"
                  alt={service.name}
                  poster={service.gallery?.find(item => item.mediaType === 'image')?.mediaUrl || DEFAULT_SERVICE_IMAGE}
                  autoplay={false} // ✅ Ne pas auto-play par défaut (économise la bande passante)
                  muted={true}
                  loop={true}
                  playsInline={true}
