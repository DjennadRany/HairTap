...
export const InstagramGallery: React.FC<InstagramGalleryProps> = ({ 
  ...
  // Obtenir le média principal d'un service (priorité aux vidéos, puis images)
  const getServiceMedia = (service: Service) => {
    if (service.gallery && service.gallery.length > 0) {
      ...
      if (video) {
        const mediaUrl = video.mediaUrl || (video as any).photoUrl;
        return {
      ...
      if (image) {
        const mediaUrl = image.mediaUrl || (image as any).photoUrl;
        return {
          url: getImageUrl(mediaUrl, DEFAULT_SERVICE_IMAGE),
          type: 'image'
        };
              ...
              >
                {media.type === 'video' ? (
                  <LazyVideo
                    ...
                    autoplay={true} // ✅ Auto-play sur mobile (Instagram-like)
                    muted={isMuted}
                    loop={true}
                    playsInline={true}
